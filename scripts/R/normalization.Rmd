...
decoyCountFiles <- list.files("../../out/counts/decoy",
                        pattern = "*decoy_featureCounts.txt$",
                        full.names = TRUE)

# Filter decoy geneids to not include rRNAs
ribosomalIDs <- read.delim("../../references/decoy/ToxoDB-68_TgondiiRH88.gff",
                           skip = 64,
                           header = F) %>% 
  filter(V3 == "rRNA") %>% 
  select(V9) %>% 
...
autoplot <- function(pc, col) {
  ...
  geom_point(size = 1, color = "#009E73FF") +
...
ggsave(
  "~/diss_plots/mrsic_stable_PCA.svg",
    plot = p1,
  device = "svg",
  width = 155/2,
  height = 115/2,
...
# --- 4) choose the stable subset (e.g., most stable 65%) ---
ord <- order(stab_score, decreasing = FALSE)
k <- round(length(ord) * 0.65)  # take central ~65% most stable
