...
```{r Heatmap all Genes, fig.height=3, fig.width=9}
...
```{r filtered Heatmap, fig.height=3, fig.width=9}
p_cutoff <- 0.05
fdr_cutoff <- 0.05
...
heatmap_sig <- Heatmap(lfc_matrix_minimal,
        column_title = "Significantly changed mito rRNA fragments",
...
When filtering based on pvalue (<=0.05) and p-adjusted (<= 0.05) only decreases in rRNA fragment expression remain.
...
# Plot volcano plots
ggplot(data = filter(volcano, contrast == "pr0_pr2"),
       aes(x = logFC,
           y = y_var)) +
...
suppressWarnings(ggplotly(volc1, tooltip = c("ID", "x", "y")))
